...
uses
  ...
  sDefaults, sGraphUtils, acAlphaImageList, acntUtils, sButton, sAlphaGraph, sMessages, sStyleSimply;
  ...
  if ImgList <> nil then
    if ImgList is TsAlphaImageList then
      ...
      if ImgList is TsCharImageList then
        Result := TsCharImageList(ImgList).Width
      else
...
begin
  if ImgList <> nil then
    if ImgList is TsAlphaImageList then
      ...
      if ImgList is TsCharImageList then
        Result := TsCharImageList(ImgList).Height
      else
...
var
  ...
  begin
    ...
    if (DrawData.Images is TsCharImageList) or (DrawData.Images is TsVirtualImageList) then begin
      C := GetFontColor(Button, SkinData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState);
...
{$ENDIF}
    ...
    if (sd <> nil) and sd.Skinned then begin
      ...
      if (DrawData.CurrentState = 0) and
        ...
        DrawData.SkinIndex := GetFontIndex(ToolBar, DrawData.SkinManager.ConstData.Sections[ssToolButton]{sd.SkinIndex}, sd.SkinManager)
      else
...
{$ENDIF}
    ...
    if (DrawData.Images is TsCharImageList) or (DrawData.Images is TsVirtualImageList) then begin
      C := GetFontColor(Button, DrawData.SkinIndex, DrawData.SkinManager, DrawData.CurrentState);
      if DrawData.Canvas <> nil then
        DrawData.Canvas.Font.Color := C;
