...
{$IFDEF DELPHI7UP}
        if (resource_string <> nil) and (LeftStr(string(resource_string), Length(fs)) = fs) then begin
{$ELSE}
        if (resource_string <> nil) and (string(resource_string) = fs) then begin
{$ENDIF}
          resource_id := i;
          Break;
        end;
  ...
  begin
    if strNewFolder <> '' then begin      // Only need to do translation once.
      Result := strNewFolder;
      Exit;
    end;
    ...
    Result := sEngName;
    strNewFolder := Result;
    if SysLocale.PriLangID = LANG_ENGLISH then Exit;

    shell_handle := SafeLoadLibrary(Shell32);
    new_folder_id := 0;

    if shell_handle > HINSTANCE_ERROR then // look for US English "New Folder" resource id.
